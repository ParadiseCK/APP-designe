;(function ($, document, window) {
    $.pictureViewer = function (options) {
        var pictureViewer_html =    '<div id="pictureViewer">' +
                                    ...
                                    '<img class="cover" src="">' +
                                    '<div><input id="chck" type="checkbox">'+
        ...
        var toPrevImage = function toPrevImage() {
            if(judegeChecked($idsArr[$nowImageIndex-1])){
        ...
        //收藏按钮
        var $switchBtn = $("#chck");
        var toStorage = function toStorage() {
            // alert($images[$nowImageIndex]);
            // alert($idsArr[$nowImageIndex]);
            var currImageId = $idsArr[$nowImageIndex];
            var checkStatus = $("input[type='checkbox']").prop("checked");
            storageImage(currImageId, checkStatus);
        };
        $switchBtn.on('click', toStorage);

        //判断当前数据是否被收藏
        if(judegeChecked($idsArr[$nowImageIndex])){
            $('input[type="checkbox"]').prop('checked', true);
        }
        else {
            $('input[type="checkbox"]').prop('checked', false);
